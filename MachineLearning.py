...
buyer = pd.read_csv("C:/Users/86178/Desktop/buyer.csv", encoding='gbk', low_memory=False)
seller = pd.read_csv("C:/Users/86178/Desktop/seller.csv", encoding='gbk', low_memory=False)
...
class solution:
    def __init__(self, seller, buyer):
        ...
        self.tem_buyer = None
        self.yixiang2value = {"第一意向": '值', '第二意向': '值.1', '第三意向': '值.2', '第四意向': '值.3',
                              '第五意向': '值.4'}
    ...
    def search_for_one_buyer(self, var, yixiangs, buyer_id, buyer_index):
        ...
        param:
            ...
            yixiangs:    该buyer的意向
            buyer_id:    该buyer的id
            buyer_index:    该buyer在self.buyer的index
        return:
            不返回值，更新剩余资源和buyer信息
        '''
        ...
        for j, (yixiang, yixiang_value) in enumerate(yixiangs):
            ...
            if yixiang == '仓库' or yixiang == '年度':
                yixiang_value = int(yixiang_value)
            tem_seller.loc[tem_seller.index, '权重1'] = tem_seller.loc[tem_seller.index, '权重1'] + (
                    tem_seller.loc[tem_seller.index, yixiang] == yixiang_value).astype(int) * weight[
                                                            self.tem_yixiang_index + j]
    ...
    def search_sameyixiang(self, emotions_and_good):
        for dis, var, sum1, rate in emotions_and_good:
            ...
            print("当前是第几意向：", self.tem_yixiang_index, "  第几轮：", "  seller数量:", len(self.tem_seller),
                  '   该意向下Buyer数量:', len(self.tem_buyer), '  买方的总数量:', sum(self.tem_buyer['购买货物数量']),
                  '   卖方总数量:',
            ...
            for i, buyer_index in enumerate(self.tem_buyer.index):
                ...
                if i == buyer_tem_len - 1:
                    print("第几个买家：", i + 1, "多少买家:", buyer_tem_len, "买家id:", buyer_id, "时间:",
    ...
    def search_by_noyixiang(self):
        for var in ["SR", "CF"]:
            ...
            for i, buyer_index in enumerate(self.tem_buyer.index):
                ...
                if i == buyer_tem_len - 1:
                    print("第几个买家：", i + 1, "多少买家:", buyer_tem_len, "买家id:", buy_id, "时间:",
                          datetime.datetime.now(), sum(self.buyer['购买货物数量']) == sum(self.seller['货物数量（张）']))
    ...
    def sort_by_xiyang(self):
        while self.tem_yixiang_index < 5:
            ...
            j = 0
            for i in emotions:
                tem1 = tem_buyer[(tem_buyer[tem_yixiang_value] == i) & (tem_buyer['品种'] == 'SR')][
                    "购买货物数量"].sum()
                tem2 = tem_buyer[(tem_buyer[tem_yixiang_value] == i) & (tem_buyer['品种'] == 'CF')][
                    "购买货物数量"].sum()
                thefirst = self.buyer[self.buyer[self.yixiang2value[self.tem_yixiang]] == i][self.tem_yixiang].iloc[0]
                if thefirst == '年度' or thefirst == '仓库':
                    j = int(i)
                    temp1 = self.tem_seller = self.seller[(self.seller[thefirst] == j) & (self.seller['品种'] == 'SR')][
                        '货物数量（张）'].sum()
                    temp2 = self.tem_seller = self.seller[(self.seller[thefirst] == j) & (self.seller['品种'] == 'CF')][
                        '货物数量（张）'].sum()
                else:
...
sol.main()
res = pd.read_csv('C:/Users/86178/Desktop/result_format_example.csv', encoding='gbk', low_memory=False)
